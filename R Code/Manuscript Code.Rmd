...
```{r Supp Table 1: p values for the Fig 1D}

chisq.test(table(metadata$age_cohort, metadata$sex))
chisq.test(table(metadata$age_cohort, metadata$raceeth))
chisq.test(table(metadata$age_cohort, metadata$new_LRTI_onset_ind))
chisq.test(data.frame(c(79,235), c(9,26))) # aggregate data available only
...
specimen_order <- genecounts %>%
  ...
  slice_head() %>%
  ungroup() %>%
  arrange(desc(age_cohort), -num_AMR) %>% # agecohort used to be study_ord
...
```{r Supp Table 2: p values for the Fig 1D}

chisq.test(table(metadata$age_cohort, metadata$abx_agly))
chisq.test(table(metadata$age_cohort, metadata$abx_bla))
chisq.test(table(metadata$age_cohort, metadata$abx_phe))
chisq.test(table(metadata$age_cohort, metadata$abx_flq))
fisher.test(table(metadata$age_cohort, metadata$abx_fcyn))
chisq.test(table(metadata$age_cohort, metadata$abx_mls))
chisq.test(table(metadata$age_cohort, metadata$abx_tet))
fisher.test(table(metadata$age_cohort, metadata$abx_ts))
...
beta_div_metadata <- metadata %>%
  ...
  distinct() %>% arrange(sample_name)

beta_diversity_genes<- genecounts %>%
  arrange(sample_name) %>% filter(AMR_yn==1) %>%
...
```{r Supp Figure 3 intubated mirror bar on Blas}
...
```{r Supp Figure 4 ARG abundance in patients with and without LRTIs}

stat.test <- metadata %>%
  mutate(LRTI_dpm = case_when(new_LRTI_onset_ind=="no LRTI" ~ "No LRTI",
                              new_LRTI_onset_ind=="CAP" ~ "LRTI",
                              new_LRTI_onset_ind=="HAP" ~ "LRTI",
                              TRUE ~ NA_character_)) %>%
  wilcox_test(sum_dpm ~ LRTI_dpm) %>%
  adjust_pvalue(method = "fdr") %>%
  add_significance("p") %>%
  add_xy_position(x = "LRTI_dpm", dodge = 0.8) %>%
  mutate(y.position = 1,
         p.adj = round(p.adj, digits=2))


LRTI_dpm <-ggplot(data = metadata %>%
  mutate(LRTI_dpm = case_when(new_LRTI_onset_ind=="no LRTI" ~ "no LRTI",
                              new_LRTI_onset_ind=="CAP" ~ "LRTI",
                              new_LRTI_onset_ind=="HAP" ~ "LRTI",
                              TRUE ~ NA_character_)) %>%
    filter(!is.na(LRTI_dpm)),
       aes(x=LRTI_dpm, y=sum_dpm +0.01)) +
                geom_jitter(position = position_jitter(seed = 1, width = 0.1, height=0), alpha=0.6, size=3) +
                geom_boxplot(alpha=0.5, width=0.4, outlier.shape = NA, fill=NA) + 
                ylab("ARG dpm\n") + xlab("\nLRTI Status") +
                scale_y_continuous(trans = log10_trans(),
                        breaks = 10^(-2:1), #trans_breaks('log10', function(x) 10^x),
                        labels = trans_format('log10', math_format(10^.x))) +
                stat_pvalue_manual(stat.test, label = "p = {p.adj}", tip.length = 0, size=4) +
                geom_hline(aes(yintercept=0)) + 
                theme(axis.line.x = element_blank(), axis.ticks.x = element_blank(),
                      legend.position = "top",
...
#ggsave("/Users/victoriachu/Documents/Research/AMR peds vs adults COVID/Results/Manuscript/AMRdpm_LRTI.pdf", LRTI_dpm, width=6, height=4)
...
logistic.display(glm(formula = AMR_yn ~ age + sex + raceeth + new_LRTI_onset_ind + ETT_day_groups, family = binomial(link = "logit"),
                                              data = metadata %>% mutate(ETT_day_groups = factor(ETT_day_groups, levels=c("<=1","2","3")),
                                                                         new_LRTI_onset_ind = factor(new_LRTI_onset_ind, levels=c("no LRTI","Indeterminate","CAP","HAP")),
                                                                         sex = factor(sex, levels = c("Male", "Female")),
                                                                         age = case_when(age == ">89" ~ "90",
                                                                                         TRUE ~ age), # for those >89 years old, represented as 90 years old for the model
                                                                         raceeth = factor(raceeth,levels=c("Non-Hispanic White", "Hispanic White", "Black or African American","Asian","Other"))) %>%
...
### Figure 3D: diff expression genera between children and adults


```{r differential expression genera by age}
...
# counting the number of 0s per row and retaining pathogens that are in >20% of all samples
...
genus<- microbiome %>%
  filter(!is.na(name) & num_bact!=0) %>%
  rename(orig_name=name) %>%
  mutate(name = gsub( " .*$", "", orig_name)) %>% select(genus_tax_id, name) %>% distinct() %>%
  mutate(genus_tax_id=as.character(genus_tax_id))

```


### Supplemental Figure 5: diff expression taxa between children and adults


```{r differential expression taxa by age}

#building countData matrix
counts <- microbiome %>%
  mutate(name=case_when(name=="[Haemophilus] ducreyi" ~ "Haemophilus ducreyi",
                                                      TRUE ~ name)) %>%
  filter(!is.na(name) & num_bact!=0) %>%
  rename(orig_name=name) %>%
  mutate(name = gsub( " .*$", "", orig_name)) %>%
  pivot_wider(id_cols=tax_id, names_from=sample_name, values_from=nt_count) %>%
  mutate(name=as.character(tax_id)) %>%
  column_to_rownames("name") %>%
  select(-tax_id) %>%
  mutate_if(is.numeric, ~replace_na(., 0))
...
sig.res_df <- as(sig.res, "data.frame") %>% rownames_to_column("Taxa") %>%
  mutate(Taxa = case_when(Taxa=="480" ~ "Moraxella catarrhalis",
                           Taxa=="75985" ~ "Mannheimia haemolytica",
                           Taxa=="747" ~ "Pasteurella multocida",
                           Taxa=="727" ~ "Haemophilus influenzae"))

diff_abundance <- ggplot(sig.res_df %>% mutate(color=case_when(log2FoldChange>=0 ~ "positive",
                                                                log2FoldChange<0 ~ "negative")),
                          aes(x=reorder(Taxa,log2FoldChange), y=log2FoldChange, fill=color)) +
  geom_bar(stat="identity", position="stack") + 
  annotate("text", x=5, y=0.5, label= "Increased in adults", color=c("#7c1730"), hjust=0, size=5) +
  annotate("segment", 
        x=4.6, xend=4.6, 
        y= 0.5, yend= 4, 
        arrow=arrow(type = "closed", length = unit(0.02, "npc")), color = "#7c1730") +
    annotate("text", x=5, y=-0.5, label= "Increased in children", color=c("#004058"), hjust=1, size=5) +
  annotate("segment", 
        x=4.6, xend=4.6, 
        y= -0.5, yend= -4, 
        arrow=arrow(type = "closed", length = unit(0.02, "npc")), color = "#004058") +
  scale_fill_manual(values=c("#004058","#7c1730")) +
  coord_flip(xlim=c(1, 5), clip="off") +
  scale_y_continuous(limits=c(-12,6), breaks=seq(-12,6,2)) +
  theme(legend.position = "none",
        plot.margin = margin(2, 2, 2, 2),
        axis.text.y = element_text(face = "italic")) +
  xlab("") + ylab("\nLog2 Fold Change")
...
taxa_freq_age <- taxa_freq_age %>% 
                           ...
                           mutate(new_name = case_when(str_detect(orig_name, 'Enterococcus') ~ gsub("Enterococcus", "E.", orig_name),
                                                       ...
                                                       TRUE ~ orig_name)) %>%
                           mutate(labels = if_else(age_cohort=="peds" & !(new_name %in% c("B. eggerthii","B. intestinalis","B. stercoris",
                                                                                          "E. columbae","M. granulomatis","P. nanceiensis",
                                                                                          "P. psychrotolerans","S. argenteus",
                                                                                          "P. versuta","S. pettenkoferi")), "", paste(" ",new_name)))
...
## Resistome with microbiome

### Figure 4: microbiome log regression peds vs adult
...
mult_forest_plot_data <- logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads + new_LRTI_onset_ind,
                                              ...
                                              data = metadata %>% mutate(age_cohort=factor(age_cohort, levels = c("peds","adult")),
                                                                         new_LRTI_onset_ind = factor(new_LRTI_onset_ind, levels =c("no LRTI","Indeterminate","CAP","HAP")),
                                                                          sum_rpm_bact = log10(sum_rpm_bact +0.01))))
...
### Figure 5: differential expression between those with AMR and those without
...
### Supplemental Table 6: logistic regression with all the differentially expressed genera

```{r Supp Table 6: binary age}
...
metadata_mNGS <- metadata %>%
  select(sample_name, age_cohort, age, AMR_yn, sum_rpm_bact, SDI_bact_reads, new_LRTI_onset_ind) %>%
  mutate(age = case_when(age == ">89" ~ "90",
                         TRUE ~ age)) %>% # for those >89 years old, represented as 90 years old for the model)
  mutate(age = as.integer(age)) %>% 
...
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads + Enterococcus + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads+ Bacteroides + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads+ Pseudomonas + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads+ Staphylococcus + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads+ Fusobacterium + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort+ sum_rpm_bact + SDI_bact_reads + Neisseria + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age_cohort + sum_rpm_bact + SDI_bact_reads+ Prevotella + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))

```


### Supplemental Table 7: cont age logistic regression with all the differentially expressed genera

```{r Supp Table 7: continuous age}

logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))

logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads + Enterococcus + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads+ Bacteroides + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads+ Pseudomonas + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads+ Staphylococcus + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads+ Fusobacterium + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age+ sum_rpm_bact + SDI_bact_reads + Neisseria + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
logistic.display(glm(formula = AMR_yn ~ age + sum_rpm_bact + SDI_bact_reads+ Prevotella + new_LRTI_onset_ind, family = binomial(link = "logit"), data = metadata_mNGS %>% mutate(sum_rpm_bact = log(sum_rpm_bact +0.01))))
